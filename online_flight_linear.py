...
# -*- coding: utf-8 -*-
"""
Neural-Fly Online Flight V8 (Linear Trajectory)
功能: 
1. 直线往返动态轨迹跟踪 (Linear Oscillation)
...
class TrajectoryGenerator:
    def __init__(self):
        ...
        # 参数配置
        self.amplitude = 2.5    # 振幅 (米) -> 单程运动距离为 5米
        self.period = 10.0      # 往返一次的时间 (秒)
        self.altitude = -1.5    # 飞行高度 (NED坐标，负数为向上)
...
async def run():
    ...
    print(f"========================================")
    print(f"   Trajectory: Linear (Back & Forth)")
    ...
    print(f">>> Linear Flight Started | Mode: {mode_str} <<<")
    ...
    while True:
        ...
        # 7. 打印
        # 显示当前 X 轴位置 (轨迹) 和 Y 轴补偿 (侧风补偿)
        print(f"\r [{status_text}] X_Pos: {curr_p[0]:5.2f}/{ref_p[0]:5.2f} | Comp_Y(Wind): {comp_acc[1]:5.2f}", end="")
