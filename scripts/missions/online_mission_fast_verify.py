...
NOTRIPLET_MODEL_PATH = os.path.join(CHECKPOINTS_DIR, "notriplet.pth")
...
class BaseOffboardControl:
    ...
    def compute(self, t):
        ...
        a_pid, a_comp, a_comp_raw = self.compute_control(cp, cv, cq, pe, ve, a_ext_wind, a_thrust_kinematic, pref, vref, aref, pwm_mean)
        ...
        # Logging
        f_est_record = self.get_logged_force(a_comp_raw)
        self.log_data.append({
            'time': t, 'p_x': cp[0], 'p_y': cp[1], 'p_z': cp[2],
            'pos_err_x': pe[0], 'pos_err_y': pe[1], 'pos_err_z': pe[2],
            'f_true_x': a_ext_wind[0]*self.MASS, 
            'f_true_y': a_ext_wind[1]*self.MASS, 
            'f_true_z': a_ext_wind[2]*self.MASS,
            'f_est_x': f_est_record[0],
            'f_est_y': f_est_record[1],
            'f_est_z': f_est_record[2],
    ...
    def get_logged_force(self, a_comp_raw):
        return a_comp_raw * self.MASS
...
class BaselineController(BaseOffboardControl):
    ...
    def get_logged_force(self, a_comp_raw):
        return self.Ki * self.pos_err_int * self.MASS
...
class L1Controller(BaseOffboardControl):
    ...
    def compute_control(self, cp, cv, cq, pe, ve, a_ext_wind, a_thrust_kinematic, pref, vref, aref, pwm_mean):
        ...
        return a_pid, self.a_comp_l1, self.a_dist_hat
...
class LearningController(BaseOffboardControl):
        ...
        if os.path.exists(ckpt_path):
            ...
            for k, v in raw_sd.items():
                if k.startswith('backbone.'):
                    state_dict[k.replace('backbone.', '', 1)] = v
                elif k.startswith('phi_net.'):
                    state_dict[k.replace('phi_net.', '', 1)] = v
    ...
    def compute_control(self, cp, cv, cq, pe, ve, a_ext_wind, a_thrust_kinematic, pref, vref, aref, pwm_mean):
        ...
        # We FREEZE P_cov at identity to prevent it from decaying to 0 and freezing the adaptation!
        # This guarantees stable and continuous learning rate throughout the 90-second flight.
        P_dot = torch.zeros_like(self.P_cov)
        ...
        # self.P_cov = self.P_cov + P_dot * self.dt  # FROZEN
        ...
        return a_pid, self.a_comp_nn_ema, a_comp_raw
...
def get_controller(controller_type, state_cache, wind_condition, model_path_override=None):
    ...
    elif controller_type == 'NoTriplet':
        path = model_path_override or NOTRIPLET_MODEL_PATH
        return LearningController(state_cache, 'NoTriplet', wind_condition, path)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--controller', type=str, required=True, 
                        choices=['Baseline','INDI','L1','Neural-Fly','FCML','NoTriplet'])
    ...
    parser.add_argument('--force_rerun', action='store_true', help='Force re-run even if data exists')
    parser.add_argument('--no_force_rerun', dest='force_rerun', action='store_false', help='Skip if data exists')
