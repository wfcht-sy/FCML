...
from config import FCML_MODEL_PATH as _FCML_MODEL_PATH, EVAL_RESULTS_DIR
...
class OffboardControl:
    ...
    def load_model(self):
        if os.path.exists(FCML_MODEL_PATH):
            ckpt = torch.load(FCML_MODEL_PATH, map_location=self.device, weights_only=True)
    ...
    def save_data(self):
        if self.log_data: pd.DataFrame(self.log_data).to_csv(os.path.join(self.results_dir, f"eval_data_FCML_{self.wind_condition}.csv"), index=False)
