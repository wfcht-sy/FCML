...
# Wind conditions to compare
WIND_CONDITIONS = [
    ("nowind",       "No Wind"),
    ("35wind",       "3.5 m/s Wind"),
    ("70wind",       "7.0 m/s Wind"),
    ("100wind",      "10.0 m/s Wind"),
    ("70p20sint",    "7.0+2.0sin(t) m/s"),
]
...
def compare_online():
    ...
    fig = plt.figure(figsize=(4 * n_wind, 10))
    fig.suptitle(
        "Online Tracking: DTW-Triplet vs MSE-only (unified backbone)",
        fontsize=14, fontweight="bold"
    )

    gs = fig.add_gridspec(2, n_wind, hspace=0.42, wspace=0.35,
    ...
    # ── Bottom row: RMSE bar chart ─────────────────────────────────────────
    ax_bar = fig.add_subplot(gs[1, :])

    wind_labels_short = [lbl.replace(" Wind", "").replace("No ", "No\n") for _, lbl in WIND_CONDITIONS]
    x = np.arange(n_wind)
    width = 0.35

    for i, ctrl in enumerate([CTRL_D, CTRL_B]):
        rmse_vals = []
        for wind_tag, _ in WIND_CONDITIONS:
            if wind_tag in data[ctrl]:
                rmse_vals.append(_rmse(data[ctrl][wind_tag]["pos_err_mag"]))
            else:
                rmse_vals.append(float("nan"))

        offset = (i - 0.5) * width
        bars = ax_bar.bar(
            x + offset, rmse_vals, width,
    ...
    ax_bar.set_xticks(x)
    ax_bar.set_xticklabels(wind_labels_short, fontsize=9)
    ax_bar.set_ylabel("Tracking RMSE (m)", fontsize=11, fontweight="bold")
    ax_bar.set_title("Online Tracking RMSE per Wind Condition", fontsize=11)
    ax_bar.legend(fontsize=9, loc="upper left", framealpha=0.9)
